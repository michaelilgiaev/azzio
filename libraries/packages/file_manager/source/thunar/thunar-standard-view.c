...
static XfceGtkActionEntry thunar_standard_view_action_entries[] =
{
    ...
    /* Azzio: <Primary>s (Ctrl+S) accelerator disabled (Edit-menu key removed; the "Select by
     * Pattern..." menu item still works by click in the hamburger-menu Edit submenu). */
    { THUNAR_STANDARD_VIEW_ACTION_SELECT_BY_PATTERN,  "<Actions>/ThunarStandardView/select-by-pattern",  "",           XFCE_GTK_MENU_ITEM,       N_ ("Select _by Pattern..."), N_ ("Select all files that match a certain pattern"),     NULL, G_CALLBACK (thunar_standard_view_select_by_pattern),           },
    /* Azzio: <Primary><shift>I (Shift+Ctrl+I) accelerator disabled (Edit-menu key removed; the
     * "Invert Selection" menu item still works by click in the hamburger-menu Edit submenu). */
                                    ...
                                    gchar              *search_query)
{
  ...
  model_folder = thunar_standard_view_model_get_folder (standard_view->model);
  if (model_folder != NULL)
    g_object_ref (G_OBJECT (model_folder)); /* temporarily hold a reference so the folder doesn't get deleted */
  thunar_standard_view_model_set_folder (standard_view->model, model_folder, search_query);
  if (model_folder != NULL)
