...
static XfceGtkActionEntry thunar_window_action_entries[] =
{
    ...
    /* Azzio: <Primary>r (Ctrl+R) accelerator disabled (View-menu key removed; the menu item still
     * works and F5/Reload keep reloading). */
    { THUNAR_WINDOW_ACTION_RELOAD,                         "<Actions>/ThunarWindow/reload",                          "",                     XFCE_GTK_IMAGE_MENU_ITEM, N_ ("_Reload"),                N_ ("Reload the current folder"),                                                    "view-refresh",            G_CALLBACK (thunar_window_action_reload),             },
    ...
    /* Azzio: F3 accelerator disabled (View-menu key removed; the menu item still toggles split view). */
    { THUNAR_WINDOW_ACTION_VIEW_SPLIT,                     "<Actions>/ThunarWindow/toggle-split-view",               "",                     XFCE_GTK_CHECK_MENU_ITEM, N_ ("S_plit View"),            N_ ("Open/Close Split View"),                                                        "view-dual",               G_CALLBACK (thunar_window_action_toggle_split_view),  },
    ...
    /* Azzio: <Primary>b (Ctrl+B) accelerator disabled (View-menu key removed; the Side Pane ->
     * Shortcuts menu item still toggles the pane). */
    ...
    /* Azzio: <Primary>e (Ctrl+E) accelerator disabled (View-menu key removed; the Side Pane ->
     * Tree menu item still toggles the pane). */
    ...
    /* Azzio: <Primary>m (Ctrl+M) accelerator disabled (View-menu key removed; the menu item still
     * toggles the menubar). The menubar toggle also lives in the window/hamburger menu, so the
     * menubar stays reachable even when hidden and without this shortcut. */
    { THUNAR_WINDOW_ACTION_VIEW_MENUBAR,                   "<Actions>/ThunarWindow/view-menubar",                    "",                     XFCE_GTK_CHECK_MENU_ITEM, N_ ("_Menubar"),               N_ ("Change the visibility of this window's menubar"),                               "open-menu",               G_CALLBACK (thunar_window_action_menubar_changed),    },
    ...
    /* Azzio: <Alt>Home accelerator disabled (Go-menu key removed; the menu item still works). */
    { THUNAR_WINDOW_ACTION_OPEN_HOME,                      "<Actions>/ThunarWindow/open-home",                       "",                     XFCE_GTK_IMAGE_MENU_ITEM, N_ ("_Home"),                  N_ ("Go to the home folder"),                                                        "go-home",                 G_CALLBACK (thunar_window_action_open_home),          },
    ...
    /* Azzio: <Alt>Up accelerator disabled (Go-menu key removed; the menu item still works). */
    { THUNAR_WINDOW_ACTION_OPEN_PARENT,                    "<Actions>/ThunarWindow/open-parent",                     "",                     XFCE_GTK_IMAGE_MENU_ITEM, N_ ("Open _Parent"),           N_ ("Open the parent folder"),                                                       "go-up",                   G_CALLBACK (thunar_window_action_go_up),              },
    /* Azzio: <Primary>l (Ctrl+L) accelerator disabled. "/" still opens the location entry (a
     * separate GTK type-ahead path, NOT this accel) and <Alt>d below is left intact. */
    { THUNAR_WINDOW_ACTION_OPEN_LOCATION,                  "<Actions>/ThunarWindow/open-location",                   "",                     XFCE_GTK_IMAGE_MENU_ITEM, N_ ("_Open Location..."),      N_ ("Specify a location to open"),                                                   NULL,                      G_CALLBACK (thunar_window_action_open_location),      },
...
thunar_window_init (ThunarWindow *window)
{
   ...
   * in the action-entry table above (accel string blanked); the zoom/scale keys (<Primary>plus,
   * <Primary>minus, <Primary>0), the view-as keys (<Primary>1/2/3) and <Primary>h (show hidden)
   * are deliberately KEPT. The window/hamburger menu keeps its View submenu (see
   * thunar_window_update_view_menu call on the popup `menu`), so the actions and their menu items
   * remain reachable without a keyboard shortcut. */
  /* Azzio: the "Go" menu is intentionally removed from the TOPBAR (menubar). The location
   * entry ("/" opens it, always-visible editable path) plus the sidebar cover navigation, so
   * the Go menu's topbar dropdown is redundant. Its accelerators <Alt>Up (open-parent),
   * <Alt>Home (open-home) and <Primary>l (open-location) are ALSO disabled in the action-entry
   * table above (accel string blanked) -- "/" and <Primary>f (search) are deliberately kept.
   * The window/hamburger menu keeps its Go submenu (see thunar_window_update_window_menu), so
   * the actions remain reachable without a keyboard shortcut. */
